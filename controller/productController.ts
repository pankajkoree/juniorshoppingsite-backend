...
export const getProductsByCategory = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(200).json({ success: true, total: fetchedProducts.length, data: fetchedProducts })
    } catch (error) {
        // <------- response for server error ------->
        return res.status(500).json({ success: false, message: "Internal server error" })
    }
...
export const getProductsByName = async (req: Request, res: Response) => {
    try {
        ...
        // splitting original entered product name into words
        ...
        if (fetchedProducts.length === 0) {
            return res.status(404).json({ success: false, message: `No matching products found for: ${productName}` })
        }
